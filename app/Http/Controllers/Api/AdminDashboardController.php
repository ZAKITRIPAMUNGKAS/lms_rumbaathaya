...
class AdminDashboardController extends BaseApiController
{
        ...
        try {
            ...
            $allActivities = $recentStudents
                ...
                ->sortByDesc(function ($activity) {
                    ...
                    // We need to get the original model's created_at
                    // For now, we'll use a timestamp field if available
                    return $activity['timestamp'] ?? 0;
                })
                ->take(10)
                ->values();
    ...
    public function charts(): JsonResponse
    {
        ...
        try {
            ...
            $charts = Cache::remember($cacheKey, 300, function () {
                ...
                // Weekly student registrations (last 7 days)
                $weeklyRegistrations = [];
                for ($i = 6; $i >= 0; $i--) {
                    $date = now()->subDays($i);
                    $count = Student::whereDate('created_at', $date->format('Y-m-d'))->count();
                    $weeklyRegistrations[] = [
                        'day' => $getDayName($date),
                        'value' => $count,
                    ];
                ...
                // Weekly material uploads (last 7 days)
                $weeklyMaterials = [];
                for ($i = 6; $i >= 0; $i--) {
                    $date = now()->subDays($i);
                    $count = Material::whereDate('created_at', $date->format('Y-m-d'))->count();
                    $weeklyMaterials[] = [
                        'day' => $getDayName($date),
                        'value' => $count,
                    ];
                ...
                // Weekly schedules added (last 7 days)
                $weeklySchedules = [];
                for ($i = 6; $i >= 0; $i--) {
                    $date = now()->subDays($i);
                    $count = Schedule::whereDate('created_at', $date->format('Y-m-d'))->count();
                    $weeklySchedules[] = [
                        'day' => $getDayName($date),
                        'value' => $count,
                    ];
                ...
                // Weekly posts published (last 7 days)
                $weeklyPosts = [];
                for ($i = 6; $i >= 0; $i--) {
                    $date = now()->subDays($i);
                ...
                // Weekly journals (last 7 days) - using BimbelJournal instead of Attendance
                $weeklyAttendances = [];
                for ($i = 6; $i >= 0; $i--) {
                    $date = now()->subDays($i);
                    $count = BimbelJournal::whereDate('date', $date->format('Y-m-d'))->count();
                    $weeklyAttendances[] = [
                        'day' => $getDayName($date),
                        'value' => $count,
                    ];
            ...
            \Log::error('Error fetching dashboard charts: ' . $e->getMessage(), [
                'trace' => $e->getTraceAsString(),
            ]);
