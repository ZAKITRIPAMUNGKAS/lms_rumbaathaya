<div x-data="chatbotWidget()"
    ...
    <button @click="isOpen = !isOpen"
        ...
        <i class="ph transition-transform duration-300"
           :class="isOpen ? 'ph-x rotate-90' : 'ph-chat-circle-dots'"></i>
    </button>
        ...
        <div class="bg-gradient-to-r from-brand-orange to-orange-600 text-white p-4 flex items-center justify-between">
            <div class="flex items-center gap-3">
                <div class="w-10 h-10 bg-white/20 rounded-full flex items-center justify-center backdrop-blur"
                     x-data="{ rotating: false }"
                     x-init="setInterval(() => { rotating = !rotating }, 2000)"
                     :class="rotating ? 'rotate-12' : '-rotate-12'"
            ...
            <template x-for="(message, index) in messages" :key="message.id">
                <div class="flex items-start gap-3"
                    ...
                    <div class="w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0"
                        ...
                        <i class="ph"
                           :class="message.isUser ? 'ph-user text-white' : 'ph-robot text-brand-orange'"></i>
                    </div>
                    <div class="flex-1"
                         :class="message.isUser ? 'flex justify-end' : ''">
                        <div class="rounded-2xl p-4 shadow-sm max-w-[80%]"
                            ...
                            <template x-if="!message.isUser && !message.text.includes('WhatsApp')">
                                <a href="https://wa.me/62812345678?text=Halo,%20saya%20ingin%20bertanya%20tentang%20Rumba%20Athaya"
                                   target="_blank"
        ...
        <div class="p-4 border-t border-gray-200 bg-white">
            ...
            <div class="mt-2 flex flex-wrap gap-2">
                ...
                @foreach($quickQuestions as $index => $q)
                    <button @click="sendMessage('{{ $q['query'] }}')"
                            :disabled="isTyping"
