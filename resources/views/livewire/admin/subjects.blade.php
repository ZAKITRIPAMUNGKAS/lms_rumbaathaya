<div class="relative min-h-screen space-y-8 p-4 sm:p-8 overflow-hidden font-sans">
    ...
    <div class="absolute top-0 left-0 w-full h-full overflow-hidden pointer-events-none -z-10 bg-[#F0F4F8]">
        <div
            class="absolute top-0 left-1/4 w-96 h-96 bg-emerald-400/30 rounded-full mix-blend-multiply filter blur-3xl opacity-70 animate-blob">
        </div>
        <div
            class="absolute top-0 right-1/4 w-96 h-96 bg-teal-400/30 rounded-full mix-blend-multiply filter blur-3xl opacity-70 animate-blob animation-delay-2000">
        </div>
        <div
            class="absolute -bottom-32 left-1/3 w-96 h-96 bg-green-300/30 rounded-full mix-blend-multiply filter blur-3xl opacity-70 animate-blob animation-delay-4000">
        </div>
    </div>

    <!-- Hero Section -->
    <div
        class="relative overflow-hidden rounded-[2.5rem] bg-gradient-to-br from-emerald-500 to-teal-600 p-8 sm:p-12 text-white shadow-2xl shadow-emerald-500/20 group">
        <div
        ...
        <div
            ...
            <div class="flex-1">
                <div
                    class="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white/20 backdrop-blur-md border border-white/20 text-xs font-bold text-emerald-100 mb-6 shadow-sm cursor-default">
                ...
                <p class="text-emerald-100 font-medium max-w-2xl text-sm sm:text-lg leading-relaxed mx-auto md:mx-0">
                    Kelola daftar mata pelajaran yang tersedia di bimbingan belajar.
                </p>
            </div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div
            ...
            <div class="relative z-10 flex items-center gap-4">
                ...
                <div>
                    <h3 class="text-3xl font-black text-slate-800">{{ $subjects->total() }}</h3>
                    <p class="text-sm font-bold text-slate-500">Total Mapel</p>
                </div>
            </div>
        </div>
    </div>
    ...
    <!-- Main Content Card -->
    <div
        class="bg-white/70 backdrop-blur-xl border border-white/60 shadow-xl shadow-emerald-500/10 rounded-[2.5rem] p-6 md:p-8 relative overflow-hidden">

        <!-- Header -->
        <div class="flex flex-col md:flex-row items-start md:items-center justify-between gap-6 mb-8">
            <div>
                <h2 class="text-2xl font-black text-slate-800">Daftar Mapel</h2>
                <p class="text-slate-500 font-medium">Semua mata pelajaran aktif.</p>
            </div>
            <button wire:click="openCreateModal"
                class="group flex items-center gap-2 px-6 py-3 rounded-xl bg-gradient-to-r from-emerald-600 to-teal-600 text-white font-bold shadow-lg shadow-emerald-500/30 hover:shadow-xl hover:-translate-y-1 hover:shadow-emerald-500/40 transition-all duration-300">
        ...
        <div class="mb-6">
            <div class="relative">
                <i
                    class="ph-bold ph-magnifying-glass absolute left-4 top-1/2 -translate-y-1/2 text-emerald-500 text-lg"></i>
                <input type="text" wire:model.live.debounce.300ms="search" placeholder="Cari mata pelajaran..."
                    class="w-full pl-11 pr-4 py-3 bg-white/50 border border-white focus:bg-white focus:ring-2 focus:ring-emerald-400 rounded-xl transition-all font-semibold text-slate-700 placeholder-slate-400">
            </div>
        </div>
        ...
        <div class="overflow-x-auto rounded-3xl border border-white/60 bg-white/30 backdrop-blur-sm">
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr
                        class="bg-emerald-50/50 border-b border-emerald-100/50 text-emerald-900 uppercase text-xs font-bold tracking-wider">
                        <th class="px-6 py-4 rounded-tl-3xl">Nama Mata Pelajaran</th>
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center justify-center gap-2">
                                    <button wire:click="openEditModal({{ $subject->id }})"
                                        class="w-8 h-8 rounded-full bg-emerald-50 text-emerald-600 hover:bg-emerald-500 hover:text-white transition-all flex items-center justify-center shadow-sm hover:shadow-emerald-500/30 group/btn"
                                        title="Edit">
                                        <i
                                            class="ph-bold ph-pencil-simple transition-transform group-hover/btn:rotate-12"></i>
                                    </button>
                                    <button wire:click="openDeleteModal({{ $subject->id }})"
        ...
        <div
                ...
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <h3 class="text-xl font-black text-slate-800 tracking-tight">
                            {{ $editingId ? 'Edit Mapel' : 'Tambah Mapel' }}</h3>
                    </div>
                </div>
            ...
            <div class="p-6">
                <form wire:submit.prevent="save" class="space-y-6">
                    <div>
                        <label class="block text-xs font-bold text-slate-700 mb-2 uppercase tracking-wide">Nama Mata
                    ...
                    <div class="flex justify-end gap-3 pt-2">
                        ...
                        <button type="submit"
                            class="px-8 py-3 rounded-xl bg-gradient-to-r from-emerald-600 to-teal-600 text-white font-bold shadow-lg shadow-emerald-500/30 hover:shadow-xl hover:-translate-y-1 transition-all">Simpan</button>
                    </div>
                </form>
            </div>
        </div>
    ...
    <div x-data="{ show: @entangle('showDeleteModal') }" x-show="show" style="display: none;"
        ...
        <div
            ...
            <h3 class="text-2xl font-black text-slate-800 mb-2">Hapus Mapel?</h3>
