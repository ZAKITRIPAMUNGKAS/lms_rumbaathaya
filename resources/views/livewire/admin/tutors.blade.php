<div class="relative min-h-screen space-y-8 p-4 sm:p-8 overflow-hidden font-sans">
    ...
    <div class="absolute top-0 left-0 w-full h-full overflow-hidden pointer-events-none -z-10 bg-[#F0F4F8]">
        <div
            class="absolute top-0 left-1/4 w-96 h-96 bg-amber-400/30 rounded-full mix-blend-multiply filter blur-3xl opacity-70 animate-blob">
        </div>
        <div
            class="absolute top-0 right-1/4 w-96 h-96 bg-orange-400/30 rounded-full mix-blend-multiply filter blur-3xl opacity-70 animate-blob animation-delay-2000">
        </div>
        <div
            class="absolute -bottom-32 left-1/3 w-96 h-96 bg-yellow-300/30 rounded-full mix-blend-multiply filter blur-3xl opacity-70 animate-blob animation-delay-4000">
        </div>
    </div>

    <!-- Hero Section -->
    <div
        class="relative overflow-hidden rounded-[2.5rem] bg-gradient-to-br from-amber-400 to-orange-500 p-8 sm:p-12 text-white shadow-2xl shadow-amber-500/20 group">
        <div
        ...
        <div
            ...
            <div class="flex-1">
                <div
                    class="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white/20 backdrop-blur-md border border-white/20 text-xs font-bold text-amber-100 mb-6 shadow-sm cursor-default">
                    <i class="ph-fill ph-chalkboard-teacher text-lg"></i>
                    <span>Manajemen Tutor</span>
                </div>
                <h1
                ...
                <p class="text-amber-100 font-medium max-w-2xl text-sm sm:text-lg leading-relaxed mx-auto md:mx-0">
                    Kelola data tutor, informasi keahlian, dan jadwal mengajar mereka.
                </p>
            </div>
        </div>
    </div>
        ...
        <div
            ...
            <div class="relative z-10 flex items-center gap-4">
                ...
                <div>
                    ...
                    <p class="text-sm font-bold text-slate-500">Total Tutor</p>
                </div>
            </div>
        </div>
        <div
            ...
            <div class="relative z-10 flex items-center gap-4">
                ...
                <div>
                    ...
                    <p class="text-sm font-bold text-slate-500">Tutor Aktif</p>
                </div>
            </div>
        </div>
        <div
            ...
            <div class="relative z-10 flex items-center gap-4">
                ...
                <div>
                    ...
                    <p class="text-sm font-bold text-slate-500">Tutor Baru</p>
                </div>
            </div>
        </div>
    </div>
    ...
    <!-- Main Content Card -->
    <div
        class="bg-white/70 backdrop-blur-xl border border-white/60 shadow-xl shadow-amber-500/10 rounded-[2.5rem] p-6 md:p-8 relative overflow-hidden">

        <!-- Header -->
        <div class="flex flex-col md:flex-row items-start md:items-center justify-between gap-6 mb-8">
            <div>
                <h2 class="text-2xl font-black text-slate-800">Daftar Tutor</h2>
                <p class="text-slate-500 font-medium">Semua data tutor terdaftar.</p>
            </div>
            <button wire:click="openCreateModal"
                class="group flex items-center gap-2 px-6 py-3 rounded-xl bg-gradient-to-r from-amber-400 to-orange-500 text-white font-bold shadow-lg shadow-amber-500/30 hover:shadow-xl hover:-translate-y-1 hover:shadow-amber-500/40 transition-all duration-300">
        ...
        <div class="flex flex-col md:flex-row gap-4 mb-6">
            <div class="relative flex-1">
                <i
                    class="ph-bold ph-magnifying-glass absolute left-4 top-1/2 -translate-y-1/2 text-amber-500 text-lg"></i>
        ...
        <div class="overflow-x-auto rounded-3xl border border-white/60 bg-white/30 backdrop-blur-sm">
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr
                        class="bg-amber-50/50 border-b border-amber-100/50 text-amber-900 uppercase text-xs font-bold tracking-wider">
                        <th class="px-6 py-4 rounded-tl-3xl">Tutor</th>
                        <th class="px-6 py-4">Bio / Keahlian</th>
                ...
                <tbody class="divide-y divide-amber-50/50">
                    @forelse($tutors as $tutor)
                        <tr class="hover:bg-white/60 transition-colors group">
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-4">
                                    <div class="relative">
                                        ...
                                        @else
                                            <div
                                                class="w-12 h-12 rounded-2xl bg-gradient-to-br from-amber-400 to-orange-500 flex items-center justify-center text-white font-bold text-lg border-2 border-white shadow-sm">
                            ...
                            <td class="px-6 py-4">
                                @if($tutor->bio)
                                    <div class="text-xs font-medium text-slate-600 italic max-w-xs line-clamp-2">
                                        "{{ $tutor->bio }}"</div>
                                @else
                                    <span class="text-xs text-slate-400">- Belum ada bio -</span>
                                @endif
                            </td>
                            <td class="px-6 py-4">
                                <span
                                    class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-emerald-50 text-emerald-700 text-xs font-bold border border-emerald-100/50">
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center justify-center gap-2">
                                    <button wire:click="openEditModal({{ $tutor->id }})"
                                    ...
                                    <button wire:click="openDeleteModal({{ $tutor->id }})"
                        ...
                        <tr>
                            <td colspan="4" class="px-6 py-12 text-center text-slate-500">
                                <div
                                    ...
                                    <i class="ph-duotone ph-chalkboard-teacher text-3xl text-slate-400"></i>
                                </div>
                                <p class="font-bold text-slate-600">Belum ada tutor</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
    ...
    <div x-data="{ show: @entangle('showModal') }" x-show="show" x-transition
        ...
        <div
            class="relative bg-white/90 backdrop-blur-2xl border border-white/50 shadow-2xl rounded-[2.5rem] w-full max-w-2xl max-h-[90vh] overflow-y-auto flex flex-col animate-modal-pop">
            <div
                class="flex items-center justify-between p-6 border-b border-slate-100 bg-white/50 backdrop-blur-md z-10">
                <div class="flex items-center gap-4">
                    <div
                        class="w-12 h-12 rounded-2xl bg-gradient-to-br from-amber-400 to-orange-500 flex items-center justify-center text-white shadow-lg shadow-amber-500/20">
            ...
            <div class="p-6 md:p-8 space-y-8">
                <form wire:submit.prevent="save" class="space-y-8">
                    <!-- Info Section -->
                    <div class="space-y-4">
                        <h4 class="flex items-center gap-2 text-sm font-bold text-amber-600 uppercase tracking-wider">
                            <i class="ph-fill ph-user-circle text-lg"></i> Informasi Personal
                        </h4>
                        <div class="bg-white/50 rounded-[2rem] p-6 border border-white/60 space-y-4">
                            ...
                            <div>
                                <label class="block text-xs font-bold text-slate-700 mb-2 uppercase tracking-wide">Bio
                                    Singkat</label>
                                <textarea wire:model="bio" rows="3"
                                    class="w-full px-5 py-3 bg-white/70 border border-slate-200 focus:border-amber-400 focus:ring-4 focus:ring-amber-100 rounded-xl transition-all font-semibold placeholder-slate-400 resize-none"></textarea>
                            </div>
                        </div>
                    </div>
                            ...
                            <div class="flex items-center gap-6">
                                ...
                                <div class="relative group flex-1">
                                    ...
                                    <label for="avatar-upload"
                                        class="block w-full border-2 border-dashed border-amber-200 rounded-xl p-8 text-center cursor-pointer hover:border-amber-400 hover:bg-amber-50 transition-all">
                                        <i class="ph-bold ph-cloud-arrow-up text-3xl text-amber-400 mb-2"></i>
                                        <p class="text-sm font-bold text-amber-600">Klik untuk upload foto</p>
                    ...
                    <div class="pt-6 border-t border-slate-200 flex justify-end gap-3">
                        ...
                        <button type="submit"
                            class="px-8 py-3 rounded-xl bg-gradient-to-r from-amber-400 to-orange-500 text-white font-bold shadow-lg shadow-amber-500/30 hover:shadow-xl hover:-translate-y-1 transition-all">
                            {{ $editingId ? 'Simpan Perubahan' : 'Tambah Tutor' }}
                        </button>
                    </div>
                </form>
            </div>
    ...
    <div x-data="{ show: @entangle('showDeleteModal') }" x-show="show" style="display: none;"
        ...
        <div
            ...
            <h3 class="text-2xl font-black text-slate-800 mb-2">Hapus Tutor?</h3>
            <p class="text-slate-500 font-medium mb-8">Data tutor yang dihapus tidak dapat dikembalikan.</p>
